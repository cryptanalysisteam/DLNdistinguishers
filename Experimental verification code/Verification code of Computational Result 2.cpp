...
int main() {
	...
	for(uint64_t iKey = 0; iKey < NK; iKey++) {
		...
		for(uint64_t iIV = 0; iIV < NIV; iIV++) {
			...
			input = input ^ GET_BIT(X[1], 7) ^ GET_BIT(X[11], 12) ^ GET_BIT(X[2], 8) ^ GET_BIT(X[8], 18) ^ GET_BIT(X[9], 26);
			...
			output = output ^ calculate_single(X[1], -Xb2, 7) ^ calculate_single(X[11], -X[12], 12) ^ calculate_single(X[2], -Xb3, 8) ^ calculate_single(X[8], -X[13], 18) ^ calculate_single(X[9], -X[14], 26);
...
//	}
	...
	double pp = epsilon(7) * epsilon(8) * epsilon(12) * epsilon(18) * epsilon(26);
