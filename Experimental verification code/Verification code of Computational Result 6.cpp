...
int main() {
	...
	for(uint64_t iKey = 0; iKey < NK; iKey++) {
		...
		for(uint64_t iIV = 0; iIV < NIV; iIV++) {
			...
			input = input ^ GET_BIT(X[0], 23) ^ GET_BIT(X[0], 19) ^ GET_BIT(X[0], 7) ^ GET_BIT(X[0], 3) ^ GET_BIT(X[0], 2) ^ GET_BIT(X[10], 18) ^
			        GET_BIT(X[10], 26) ^ GET_BIT(X[10], 25) ^ GET_BIT(X[10], 24) ^ GET_BIT(X[1], 16) ^ GET_BIT(X[11], 28) ^ GET_BIT(X[11], 20) ^
			        GET_BIT(X[11], 19) ^ GET_BIT(X[2], 12) ^ GET_BIT(X[2], 8) ^ GET_BIT(X[2], 24) ^ GET_BIT(X[2], 23) ^ GET_BIT(X[8], 24) ^ GET_BIT(X[8], 23) ^
			        GET_BIT(X[8], 12) ^ GET_BIT(X[8], 11) ^ GET_BIT(X[3], 31) ^ GET_BIT(X[3], 28) ^ GET_BIT(X[3], 20) ^
			        GET_BIT(X[3], 16) ^ GET_BIT(X[3], 7) ^ GET_BIT(X[3], 4) ^ GET_BIT(X[3], 12) ^ GET_BIT(X[3], 11);
			...
			output = output ^ calculate_single(X[0], -Xb1, 23) ^ calculate_single(X[0], -Xb1, 19) ^ calculate_single(X[0], -Xb1, 7) ^ calculate_two(X[0], -Xb1, 3) ^
			         calculate_single(X[10], -X[15], 18) ^ calculate_three(X[10], -X[15], 26) ^ calculate_single(X[1], -Xb2, 16) ^ calculate_single(X[11], -X[12], 28) ^ calculate_two(X[11], -X[12], 20) ^
			         calculate_single(X[2], -Xb3, 12) ^ calculate_single(X[2], -Xb3, 8) ^ calculate_two(X[2], -Xb3, 24) ^ calculate_two(X[8], -X[13], 24) ^ calculate_two(X[8], -X[13], 12) ^
			         calculate_single(X[3], -Xb4, 31) ^ calculate_single(X[3], -Xb4, 28) ^ calculate_single(X[3], -Xb4, 20) ^ calculate_single(X[3], -Xb4, 16) ^
			         calculate_single(X[3], -Xb4, 7) ^ calculate_single(X[3], -Xb4, 4) ^ calculate_two(X[3], -Xb4, 12);
...
//	}
	...
	double pp = epsilon(23) * epsilon(19) * epsilon(7) * epsilon(3) * epsilon(18) * epsilon3(26) * epsilon(16) * epsilon(28) *
	            epsilon(20) * epsilon(12) * epsilon(8) * epsilon(24) * epsilon(24) * epsilon(12) * epsilon(31) * epsilon(28) *
				epsilon(20) * epsilon(16) * epsilon(7) * epsilon(4) * epsilon(12) ;
