...
int main() {
	...
	for(uint64_t iKey = 0; iKey < NK; iKey++) {
		...
		for(uint64_t iIV = 0; iIV < NIV; iIV++) {
			...
			input = input ^ GET_BIT(X[0], 19) ^ GET_BIT(X[0], 18) ^ GET_BIT(X[0], 12) ^ GET_BIT(X[0], 7) ^ GET_BIT(X[0], 6) ^ GET_BIT(X[8], 7) ^
			        GET_BIT(X[2], 24) ^ GET_BIT(X[10], 26) ^ GET_BIT(X[10], 12) ^ GET_BIT(X[10], 6) ^
			        GET_BIT(X[3], 20) ^ GET_BIT(X[3], 19) ^ GET_BIT(X[3], 16) ^ GET_BIT(X[3], 12) ^ GET_BIT(X[11], 31) ^ GET_BIT(X[11], 30) ^
			        GET_BIT(X[11], 19) ^ GET_BIT(X[11], 16) ^ GET_BIT(X[11], 15);
			...
			output = output ^ calculate_two(X[0], -Xb1, 19) ^ calculate_single(X[0], -Xb1, 12) ^ calculate_two(X[0], -Xb1, 7) ^ calculate_single(X[8], -X[12], 7) ^
			         calculate_single(X[2], -Xb3, 24) ^ calculate_single(X[10], -X[14], 26) ^ calculate_single(X[10], -X[14], 12) ^ calculate_single(X[10], -X[14], 6) ^
			         calculate_two(X[3], -Xb4, 20) ^ calculate_single(X[3], -Xb4, 16) ^ calculate_single(X[3], -Xb4, 12) ^
			         calculate_two(X[11], -X[15], 31) ^ calculate_single(X[11], -X[15], 19) ^ calculate_two(X[11], -X[15], 16) ;
...
//	}
	...
	double pp = epsilon(7) * epsilon(12) * epsilon(19) * epsilon(7) * epsilon(24) * epsilon(6) * epsilon(12) * epsilon(26) *
	            epsilon(12) * epsilon(16) * epsilon(20) * epsilon(16) * epsilon(19) * epsilon(31) ;
