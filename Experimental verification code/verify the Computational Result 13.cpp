...
int main() {
	printf("For the Computational Result 13, we randomly generated 2^{6} keys and 2^{32} IVs\n");
	uint64_t NK;
	uint64_t NIV;
	NK=((uint64_t)1<<6);
	NIV=((uint64_t)1<<32);
	...
	for(uint64_t iKey = 0; iKey < NK; iKey++) {
		...
		for(uint64_t iIV = 0; iIV < NIV; iIV++) {
			...
			output =
			    ...
			    ((X[7] >> 12) & 0x1) ^         
			    ((X[8] >> 0) & 0x1) ^           
			    (((X[6] >> 31) & 0x1) ^ ((X[6] >> 19) & 0x1)) ^  
			    (((X[11] >> 19) & 0x1) ^ ((X[11] >> 7) & 0x1)) ^
			    ((X[10] >> 12) & 0x1) ^         
			    ((X[15] >> 12) & 0x1) ^         
			    ((X[3] >> 0) & 0x1) ^           
			    ((X[14] >> 16) & 0x1) ^         
			    ((((X[10] >> 11) & 0x1) ^ 0x1) & ((X[15] >> 11) & 0x1)) ^
			    ((((X[10] >> 11) & 0x1) ^ ((X[15] >> 11) & 0x1) ^ 0x1) & (((X[10] >> 10) & 0x1) ^ 0x1) & ((X[15] >> 10) & 0x1));
...
//	}
	printf("Computational Result 14 is %f(2^%f)\n",res,log2(abs(res)));
